...
originalImage = imread("final 1\1-751\Stone- (443).jpg");
...
gamma = 1.0; % start gamma from 1.0
...
while gamma <= 5.0
    ...
    for i = 1:regionSize:size(filteredImage, 1)
        for j = 1:regionSize:size(filteredImage, 2)
            % แยกพื้นที่ท้องถิ่น
            localRegion = filteredImage(i:min(i+regionSize-1, size(filteredImage, 1)), j:min(j+regionSize-1, size(filteredImage, 2)));
            ...
            enhancedImage(i:min(i+size(localRegion,1)-1, size(filteredImage, 1)), j:min(j+size(localRegion,2)-1, size(filteredImage, 2))) = localEnhancedRegion;
        end
    end
    ...
    title(['Enhanced Image (Local Power Law Transformation) - Gamma: ' num2str(gamma)]);
    ...
    if ~isempty(stats)
        ...
        newBoundingBox = [largestBoundingBox(1), largestBoundingBox(2) - largestBoundingBox(4), largestBoundingBox(3)+10, newHeight];
        ...
        while scaleFactor <= 5
            ...
            leftKidney = enhancedImage(leftRegion(2):min(leftRegion(2) + leftRegion(4)-1, size(enhancedImage, 1)), leftRegion(1):min(leftRegion(1) + leftRegion(3)-1, size(enhancedImage, 2)));
            rightKidney = enhancedImage(rightRegion(2):min(rightRegion(2) + rightRegion(4)-1, size(enhancedImage, 1)), rightRegion(1):min(rightRegion(1) + rightRegion(3)-1, size(enhancedImage, 2)));
        ...
        % เพิ่ม gamma สำหรับการวนลูปครั้งต่อไป
        gamma = gamma + 0.1; % ปรับการเพิ่มค่าตามต้องการ
    else
        fprintf('No kidney stones detected. Continue monitoring your health.\n');
        break; % ออกจากลูปเมื่อไม่พบวัตถุใดๆ
    end
end
